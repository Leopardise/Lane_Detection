...
def detect_lane_contours_manual(edges, original_image):
    kernel = np.ones((5, 5), np.uint8)
    edges_closed = np.maximum(edges, np.minimum(edges, edges))  # Manual closing
    edges_dilated = np.maximum(edges_closed, np.minimum(edges_closed, edges_closed))  # Manual dilation

    height, width = edges_dilated.shape
    contours = []
    visited = np.zeros_like(edges_dilated, dtype=bool)
    
    for y in range(height):
        for x in range(width):
            if edges_dilated[y, x] > 0 and not visited[y, x]:
                stack = [(x, y)]
                contour = []
                while stack:
                    cx, cy = stack.pop()
                    ...
                    for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        nx, ny = cx + dx, cy + dy
                        if 0 <= nx < width and 0 <= ny < height and edges_dilated[ny, nx] > 0:
                            stack.append((nx, ny))
                if len(contour) > 100:
    ...
    for contour in contours:
        for point in contour:
            contour_image[point[1], point[0]] = [0, 255, 0]  # Manually marking contours
    
    return contour_image, contours

def fit_quadratic_manual(contours, original_image):
    image_with_lanes = np.copy(original_image)
    for contour in contours:
        points = np.array(contour)
        x_coords, y_coords = points[:, 0], points[:, 1]
        sorted_indices = np.argsort(y_coords)
        x_coords, y_coords = x_coords[sorted_indices], y_coords[sorted_indices]

        unique_y, unique_indices = np.unique(y_coords, return_index=True)
        x_coords = x_coords[unique_indices]
        y_coords = unique_y

        if len(x_coords) > 12:
            try:
                A = np.vstack([y_coords**2, y_coords, np.ones(len(y_coords))]).T
                coeffs, _, _, _ = np.linalg.lstsq(A, x_coords, rcond=None)
                x_fit = coeffs[0] * y_coords**2 + coeffs[1] * y_coords + coeffs[2]

                for i in range(len(y_coords) - 1):  # Manually drawing the curve
                    x1, y1 = int(x_fit[i]), int(y_coords[i])
                    x2, y2 = int(x_fit[i + 1]), int(y_coords[i + 1])
                    if 0 <= x1 < image_with_lanes.shape[1] and 0 <= x2 < image_with_lanes.shape[1]:
                        for thickness in range(-2, 3):  # Increase thickness
                            if 0 <= x1 + thickness < image_with_lanes.shape[1]:
                                image_with_lanes[y1, x1 + thickness] = [255, 0, 0]
                            if 0 <= x2 + thickness < image_with_lanes.shape[1]:
                                image_with_lanes[y2, x2 + thickness] = [255, 0, 0]
            except Exception as e:
                print(f"Manual quadratic fitting error: {e}")
    return image_with_lanes

def bgr_to_rgb(image):
    return image[:, :, ::-1]  # Reverse the last axis

def visualize_steps(original_image, edges, contour_image, final_lane_image, output_path):
...
def detect_lane_contours_manual(edges, original_image):
    ...
    kernel = np.ones((5, 5), np.uint8)
    edges_dilated = manual_dilate(edges, kernel)
    ...
    for contour in contours:
        for point in contour:
            if 0 <= point[1] < contour_image.shape[0] and 0 <= point[0] < contour_image.shape[1]:
                contour_image[point[1], point[0]] = [0, 255, 0]  # Manually marking contours
...
def process_all_images(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(('.png', '.jpg', '.jpeg')):
            ...
            # Load the processed edge image and original image
            manual_resize(temp_files[13], temp_files[13])
            edges = cv2.imread(temp_files[13])
            manual_resize(input_path, input_path)
            original_image = cv2.imread(input_path)
            
            # Detect lane contours and fit quadratic curves
            contour_image, contours = detect_lane_contours_manual(edges, original_image)
            final_lane_image = fit_quadratic_manual(contours, original_image)
            
            # Save final output
            visualize_steps(original_image, edges, contour_image, final_lane_image, visualization_output_path)
            print(f"Visualization saved at: {visualization_output_path}")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python3 main.py <input_img_dir> <output_img_dir>")
        sys.exit(1)
    task = sys.argv[1]
    input_img_dir = sys.argv[2]
    output_img_dir = sys.argv[3]
    process_all_images(input_img_dir, output_img_dir)
